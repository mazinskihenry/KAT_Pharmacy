...
private _placed = _target getVariable ["kat_medical_circulation_IVplaced", false];
private _site = _target getVariable ["kat_medical_circulation_IVsite", 0];

if (_target getVariable [QEGVAR(circulation,IVplaced), false] && (_target getVariable [QEGVAR(circulation,IVsite), 0] == _selectionN)) then {
    private _a = _target getVariable [QEGVAR(circulation,usedIV), ""];
    if !(_a isEqualTo "") then {
        private _text = format ["STR_KAT_circulation_%1_Display", _a];
